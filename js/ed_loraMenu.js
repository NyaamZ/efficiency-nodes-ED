...
const loadImageList = async (type) => {
	imagesByType[type] = await (await api.fetchApi(`/ed_nodes/images/${type}`)).json();
};
...
app.registerExtension({
	...
	init() {		
		app.ui.settings.addSetting({
			"id": "ed.Thumbnails.DisplayMode",
			"category": [
				"Appearance",
				"Thumbnails",
				"DisplayMode"
			],
			"name": "💬ED Thumbnails display mode",
			"type": "combo",
			"options": [
				"List (Normal)",
				"Tree (Subfolders)",
			...
			"defaultValue": "Thumbnails (grid)"
		});
		
		$el("style", {
			textContent: `
				.ed_nodes-combo-image {
					position: absolute;
					left: 0;
					top: 0;
				...
				.ed_nodes-combo-image.left {
					object-position: top right;
				}
				.ed_nodes-combo-folder { opacity: 0.7 }
				.ed_nodes-combo-folder-arrow { display: inline-block; width: 15px; }
				.ed_nodes-combo-folder:hover { background-color: rgba(255, 255, 255, 0.1); }
				.ed_nodes-combo-prefix { display: none }

				/* Special handling for when the filter input is populated to revert to normal */
				.litecontextmenu:has(input:not(:placeholder-shown)) .ed_nodes-combo-folder-contents {
					display: block !important;
				}
				.litecontextmenu:has(input:not(:placeholder-shown)) .ed_nodes-combo-folder { 
					display: none;
				}
				.litecontextmenu:has(input:not(:placeholder-shown)) .ed_nodes-combo-prefix { 
					display: inline;
				}
				...
				.ed_nodes-combo-grid .comfy-context-menu-filter {
					grid-column: 1 / -1;
					position: sticky;
					top: 0;
				}
				.ed_nodes-combo-grid .litemenu-entry {
				...
				.ed_nodes-combo-grid .litemenu-entry:before {
					content: "";
					display: block;
					width: 100%;
					height: 250px;
		...
		const imageHost = $el("img.ed_nodes-combo-image");
		...
		const updateMenu = async (menu, type, overWidget) => {
			try {
				...
				console.error(error);
				console.error("Error loading ed.betterCombos data")
			}
			...
			const addImageHandler = (item) => {
				...
				if (images[text]) {
					...
					item.addEventListener(
						"mouseover",
						() => {
							imageHost.src = `/ed_nodes/view/${encodeRFC3986URIComponent(images[text])}?${+new Date()}`;
			...
			const createTree = () => {
				...
				for (const item of items) {
					...
					if (path.length > 1) {
						...
						const prefix = $el("span.ed_nodes-combo-prefix", {
				...
				const createFolderElement = (name) => {
					const folder = $el("div.litemenu-entry.ed_nodes-combo-folder", {
						innerHTML: `<span class="ed_nodes-combo-folder-arrow">▶</span> ${name}`,
				...
				const insertFolderStructure = (parentElement, map, level = 0) => {
					for (const [folderName, content] of map.entries()) {
						...
						const childContainer = $el("div.ed_nodes-combo-folder-contents", {
							style: { display: "none" },
						});
						...
						folderElement.addEventListener("click", (e) => {
							e.stopPropagation();
							const arrow = folderElement.querySelector(".ed_nodes-combo-folder-arrow");
			...
			const addImageData = (item) => {
				const text = item.getAttribute("data-value").trim();
				if (images[text]) {
					item.style.setProperty("--background-image", `url(/ed_nodes/view/${encodeRFC3986URIComponent(images[text])}${dateNow})`);
				}
			};
			...
			const displaySetting = app.ui.settings.getSettingValue("ed.Thumbnails.DisplayMode");
			const splitBy = (navigator.platform || navigator.userAgent).includes("Win") ? '\\' : '/';
			const dateNow = "?t=" + Date.now();
			let folderTree = {};
			let folderList = [];

			if (displaySetting === "List (Normal)") {
				for (const item of items) {
					addImageHandler(item);
				}
			} else if (displaySetting === "Tree (Subfolders)") {
				createTree();
			} else {
				const widget_folder = overWidget.value.substring(0, overWidget.value.lastIndexOf(splitBy));				
				menu.classList.add("ed_nodes-combo-grid");
